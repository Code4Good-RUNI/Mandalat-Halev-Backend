...
// Helper for Salesforce contact lookup with automatic token management
const { withSalesforceRetry } = require('./salesforceRetry');

async function getContactByIdAndEmail(userId, email) {
  try {
    return await withSalesforceRetry(async (conn) => {
      const result = await conn.query(
        `SELECT Id, Email FROM Contact WHERE Id = '${userId}' AND Email = '${email}' LIMIT 1`
      );
      return result.records[0] || null;
    ...
    console.error('Error in getContactByIdAndEmail:', error);
    return null;
  }
}
